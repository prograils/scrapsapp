class DashboardController < ApplicationController
  ...
  def index
    ...
    org_ids = current_user.organizations.to_a.map(&:id) || []
    #org_ids.delete(current_user.private_organization.id)
    observed_ids = current_user.observed_organizations.to_a.map(&:id) || []
  ...
  def starred
    my_organizations = current_user.organizations.to_a.map(&:id) || []
  ...
  def observed
    observed_ids = current_user.observed_organizations.to_a.map(&:id) || []
    my_organizations = current_user.organizations.to_a.map(&:id) || []
    user_ids = current_user.observed_users.to_a.map(&:id) || []
